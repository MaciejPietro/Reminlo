...
public sealed record GetAllWorkspacesQuery() : IRequest<ErrorOr<List<WorkspaceListDto>>>;

internal sealed class GetAllWorkspacesQueryHandler(
    IWorkspaceRepository workspaceRepository,
    IUserService userService
) : IRequestHandler<GetAllWorkspacesQuery, ErrorOr<List<WorkspaceListDto>>>
{
    public async Task<ErrorOr<List<WorkspaceListDto>>> Handle(GetAllWorkspacesQuery request, CancellationToken cancellationToken)
    {
